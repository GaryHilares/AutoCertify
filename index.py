...
def register():
    ...
    if database["certifiers"].find_one({"name": name}) is not None:
        return render_template('error.html', message=f"An account with the name {name} already exists.")
    ...
    return render_template('success.html', message=f"Account {name} created successfully")
...
def verify_account():
    ...
    if not check_metadata(url, 'ca-key', f'ca-key-{name}'):
        return render_template('error.html', message="Metadata tag was not found. Make sure you followed the steps correctly."), 400
    
    # Update database with verified URL
    database["certifiers"].update_one({'_id': certifier["id"]}, {"$set": {"url": url}})

    # Return success message
    return render_template('success.html', message=f"Account {name} verified correctly with the website {url}")
...
def create_certificate():
    ...
    if not certifier_name or not certifier_password or not certificate_name or not certificate_title:
        return render_template('error.html', message="A field is missing in your request."), 400
    ...
    if not certifier or not bcrypt.check_password_hash(certifier["password"], certifier_password):
        return render_template('error.html', message="Certifier credentials are incorrect."), 403
    ...
    # Return success message
    return render_template('success.html', message=f"Certificate with the id {insert_data.inserted_id} created successfully")
...
def view_certificate():
    ...
    return render_template('view-certificate.html', certificate={
        "id": certificate["_id"],
